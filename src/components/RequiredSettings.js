...
import {Modal, View, KeyboardAvoidingView, StyleSheet, Text, TouchableOpacity, Platform} from 'react-native';
...
export default class RequiredSettings extends Component {
    ...
    shouldShowSettings = (user, keyphrase) => {
        return !user.name || !user.email || !user.room || !user.duty || !user.keyphrase || user.keyphrase !== keyphrase;
    };
    ...
    render() {
        ...
        return (
            <Modal
                visible={shouldShow}>
                <SafeAreaView style={{ flex:0, backgroundColor: colors.inactiveTabColor }} />
                        ...
                        <View style={styles.innerContainer}>
                            ...
                            <View style={styles.buttonContainer}>
                                <TouchableOpacity style={styles.button} onPress={this.onDonePress}>
                                    <Text
                                        style={styles.buttonText}>{strings('settings.change_password_modal_ok')}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>)
                    }
