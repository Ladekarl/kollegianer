...
export default class OverviewScreen extends Component {
  ...
  _getNearestBirthday = (b1, b2) => {
    ...
    if (
      b1.getMonth() < today.getMonth() ||
      (b1.getMonth() === today.getMonth() && b1.getDate() < today.getDate())
    ) {
      b1Copy.setFullYear(today.getFullYear() - 1);
    }
    if (
      b2.getMonth() < today.getMonth() ||
      (b2.getMonth() === today.getMonth() && b2.getDate() < today.getDate())
    ) {
  ...
  componentDidMount() {
      ...
      .then(snapshot => {
        ...
        snapshot.forEach(snap => {
          ...
          if (user.birthday) {
            ...
            if (birthdayYear < 100) {
              birthdayYear =
                birthdayYear > 50 ? '19' + birthdayYear : '20' + birthdayYear;
            ...
            if (
              nextBirthdayUsers.length === 0 ||
              this._getNearestBirthday(userBirthday, nextBirthdayDate) ===
                userBirthday
            ) {
              if (
                nextBirthdayDate &&
        ...
        LocalStorage.getUser()
          .then(localUser => {
            ...
            snapshot.forEach(snap => {
              ...
              if (
                this.remoteUser &&
                this.remoteUser.kitchenweek &&
                this.remoteUser.sheriff
              ) {
    ...
    Database.listenFish(snapshot => {
      ...
      this.setState({
        fish:
          this.days[fishTime.getDay()] +
          ' ' +
          fishTime.getDate() +
          ' ' +
          this.months[fishTime.getMonth()] +
          ' ' +
          fishTime.getHours() +
          ':' +
          fishTime.getMinutes(),
  ...
  onMvpSubmit = () => {
    Database.updateEvent('mvp', this.state.selectedMvp).catch(error =>
      console.log(error),
    );
    this.setMvpModalVisible(false);
  };
  ...
  onShotsSubmit = () => {
    Database.updateEvent('shots', this.state.selectedShots).catch(error =>
      console.log(error),
    );
    this.setShotsModalVisible(false);
  };
  ...
  showBeerpongAlert = () => {
    Alert.alert(
      ...
      [
        {
          text: strings('overview.modal_cancel'),
          onPress: () => {},
        },
  ...
  updateBeerPongEvent = () => {
    ...
    Database.updateEvent('beerpong', beerpong).catch(error =>
      console.log(error),
    );
  };
  ...
  updatePartyMode = () => {
    ...
    return Database.updateEvent(
      'partymode',
      partymode.length > 0 ? '' : this.localUser.name,
    ).catch(error => console.log(error));
  };

  updateFoxEvent = () => {
    Database.updateEvent('fox', !this.state.events.fox).catch(error =>
      console.log(error),
    );
  };
  ...
  _renderBirthdayNames = () => {
    ...
    for (let i = 0; i < this.state.nextBirthdayUsers.length; i++) {
      ...
      birthdayNames.push(
        <Text key={nextBirthDayUser.room} numberOfLines={1} style={styles.text}>
          ...
          <View style={styles.rowContainer}>
            <TouchableOpacity
              ...
              <Text style={styles.whiteText}>
                {strings('overview.beerpong_button')}
              </Text>
              <Image
                resizeMode="contain"
                style={styles.image}
              ...
              <Text numberOfLines={2} style={styles.whiteText}>
                {beerpong
                  ? strings('overview.beerpong_button_true')
                  : strings('overview.beerpong_button_false')}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={this.columnContainerStyle(
                partymode && partymode.length > 0,
              )}
              onPress={this.updatePartyMode}>
              <Text style={styles.whiteText}>
                {strings('overview.partymode_button')}
              </Text>
              <Image
                resizeMode="contain"
                style={styles.image}
              ...
              <Text numberOfLines={2} style={styles.whiteText}>
                {partymode && partymode.length > 0
                  ? partymode
                  : strings('overview.partymode_button_false')}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              <Text numberOfLines={2} style={styles.whiteText}>
                {fox
                  ? strings('overview.fox_button_true')
                  : strings('overview.fox_button_false')}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
